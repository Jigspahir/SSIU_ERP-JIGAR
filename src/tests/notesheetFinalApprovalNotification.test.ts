...
export async function runNotesheetFinalApprovalNotificationTests(): Promise<void> {
  ...
  [facultyUser, hodUser, hoiUser, dyRegUser, registrarUser, unrelatedFaculty].forEach(u => {
  ...
  // ─── STEP 5: REGISTRAR FINAL APPROVAL ─────────────────────────────────────
  ...
  assert(finalNote.status === 'APPROVED', '5.1 Notesheet status is APPROVED / COMPLETED');
  assert(finalNote.decision === 'APPROVED', '5.2 Notesheet decision is APPROVED');
  assert(finalNote.currentOffice === 'COMPLETED', '5.3 currentOffice is COMPLETED');
  assert(Boolean(finalNote.finalApprovalId), `5.4 Digital Approval ID stored: ${finalNote.finalApprovalId}`);

  // ─── STEP 6: VERIFY NOTIFICATION DELIVERED TO ALL 5 PARTICIPANTS ──────────
  console.log('\n--- Step 6: Verify Notification Delivered to Participants ---');
  ...
  assert(facFinalNotifs.length > 0, '5.1 [Creator] Demo Faculty 1 received Final Approval notification');
  if (facFinalNotifs.length > 0) {
    assert(facFinalNotifs[0].title === 'Notesheet Approved', '5.1b Notification title is "Notesheet Approved"');
    assert(facFinalNotifs[0].message.includes('successfully approved'), '5.1c Notification message contains completion text');
    assert(facFinalNotifs[0].message.includes(newNote.noteSheetNumber), '5.1d Notification message contains Notesheet number');
  }
  ...
  assert(hodFinalNotifs.length > 0, '5.2 [Participant] Demo HOD 1 received Final Approval notification');
  ...
  assert(hoiFinalNotifs.length > 0, '5.3 [Participant] Demo HOI 1 received Final Approval notification');
  ...
  assert(dyRegFinalNotifs.length > 0, '5.4 [Participant] Demo Deputy Registrar 1 received Final Approval notification');
  ...
  assert(regFinalNotifs.length > 0, '5.5 [Approver] Demo Registrar 1 received Final Approval notification');

  // ─── STEP 6: VERIFY UNRELATED USERS RECEIVE ZERO NOTIFICATIONS ────────────
  console.log('\n--- Step 6: Verify Unrelated Users Are NOT Spammed ---');
  const unrelatedNotifs = db.getNotifications(unrelatedFaculty, 'FACULTY').filter(n => n.referenceId === newNote.noteSheetNumber);
  assert(unrelatedNotifs.length === 0, '6.1 Unrelated faculty member received 0 notifications');

  // ─── STEP 7: VERIFY PENDING WITH ME CLEARED FOR ALL ───────────────────────
  console.log('\n--- Step 7: Verify Pending With Me Cleared ---');
  assert(db.getPendingWithMeNotesheets(facultyUser, 'FACULTY').filter(n => n.id === newNote.id).length === 0, '7.1 Faculty Pending With Me is 0');
  assert(db.getPendingWithMeNotesheets(hodUser, 'HOD').filter(n => n.id === newNote.id).length === 0, '7.2 HOD Pending With Me is 0');
  assert(db.getPendingWithMeNotesheets(hoiUser, 'PRINCIPAL').filter(n => n.id === newNote.id).length === 0, '7.3 HOI Pending With Me is 0');
  assert(db.getPendingWithMeNotesheets(dyRegUser, 'DEPUTY_REGISTRAR').filter(n => n.id === newNote.id).length === 0, '7.4 Deputy Registrar Pending With Me is 0');
  assert(db.getPendingWithMeNotesheets(registrarUser, 'REGISTRAR').filter(n => n.id === newNote.id).length === 0, '7.5 Registrar Pending With Me is 0');
