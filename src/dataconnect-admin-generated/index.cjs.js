...
const connectorConfig = {
  connector: 'example',
  serviceId: 'swarrnim-erp-prod-service',
...
function createBulkOperation(dcOrVarsOrOptions, varsOrOptions, options) {
  const { dc: dcInstance, vars: inputVars, options: inputOpts} = validateAdminArgs(connectorConfig, dcOrVarsOrOptions, varsOrOptions, options, true, true);
  dcInstance.useGen(true);
  return dcInstance.executeMutation('CreateBulkOperation', inputVars, inputOpts);
}
...
function listBulkOperations(dcOrOptions, options) {
  const { dc: dcInstance, options: inputOpts} = validateAdminArgs(connectorConfig, dcOrOptions, options, undefined);
  dcInstance.useGen(true);
  return dcInstance.executeQuery('ListBulkOperations', undefined, inputOpts);
}
exports.listBulkOperations = listBulkOperations;
